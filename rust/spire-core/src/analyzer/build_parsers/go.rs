...
pub fn parse_go_mod(project_root: &Path) -> Option<BuildMetadata> {
    ...
    Some(BuildMetadata {
        ...
        scripts: vec![
            BuildScript { name: "build".to_string(), command: "go build ./...".to_string(), tool_call: Some(serde_json::json!({"tool": "project/build", "args": {"mode": "build"}})) },
            BuildScript { name: "test".to_string(), command: "go test ./...".to_string(), tool_call: Some(serde_json::json!({"tool": "project/build", "args": {"mode": "test"}})) },
            BuildScript { name: "vet".to_string(), command: "go vet ./...".to_string(), tool_call: Some(serde_json::json!({"tool": "project/build", "args": {"mode": "vet"}})) },
            BuildScript { name: "fmt".to_string(), command: "go fmt ./...".to_string(), tool_call: Some(serde_json::json!({"tool": "project/build", "args": {"mode": "fmt"}})) },
        ],
        features: vec![],
        targets: vec![],
