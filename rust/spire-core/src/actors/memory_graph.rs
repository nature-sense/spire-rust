...
use crate::models::memory_graph::{
    ContextSearchResult, GraphEdge, GraphNode, McpConfigFile, McpServerConfigEntry, MemoryEntry,
    MemoryMetadata, NodeFilter, NodeInput, NodeType, NodeUpdate, ProjectSnapshot, ProjectStats,
    RelationshipInput, RelationshipType, RetrievalSource, SchemaError, ScoredNode, SearchOptions,
    TraversalDirection, TraversalOptions, TraversalResult,
};
...
pub enum MemoryGraphMessage {
    ...
    /// Reads the file, parses it, and stores each server as a SpireMcpConfig node.
    BootstrapMcpConfig {
        config_path: PathBuf,
        reply_to: tokio::sync::oneshot::Sender<Result<()>>,
    },
    ...
    /// Import MCP server config from a JSON file (replaces existing).
    ImportMcpConfigFile {
        config_path: PathBuf,
        reply_to: tokio::sync::oneshot::Sender<Result<()>>,
    },
    /// Save (create or update) a single MCP server config entry.
    SaveMcpConfig {
        entry: McpServerConfigEntry,
        reply_to: tokio::sync::oneshot::Sender<Result<()>>,
    },
    /// Delete a single MCP server config entry by name.
    DeleteMcpConfig {
        name: String,
        reply_to: tokio::sync::oneshot::Sender<Result<()>>,
    },
...
impl MemoryGraphActor {
    ...
    fn init_graph(&mut self, data_dir: &PathBuf) -> Result<()> {
        ...
        let graph_db = match GraphDb::recover(data_dir, graph_id) {
            ...
            Err(e) => {
                ...
                // The mismatch means the WAL header's anchor sequence (seq=0 from
                // initial creation) doesn't match the snapshot sequence (seq=1, 2, ...)
                // because write_snapshot() never rotates the WAL header. We must
                // remove only the WAL — the snapshot files are valid and contain
                ...
                if msg.contains("wal snapshot sequence") && msg.contains("does not match applied snapshot") {
                    ...
                    // NOTE: We intentionally keep all snapshot files. The snapshot
                    // contains the full graph state up to its sequence number.
                    // After removing the stale WAL, recover() will apply the latest
                    // snapshot and create a fresh WAL properly anchored to that
                    // snapshot's sequence, preserving all persisted graph data.
                }

                // Fall back to recovering from the directory with stale WAL removed.
                // recover() will apply the latest snapshot (if any) and create a
                // fresh WAL anchored to that snapshot's sequence, preserving all
        ...
        // This task listens on the snapshot_rx channel and writes a snapshot
        // to disk after a 2-second debounce window following each mutation.
        if let Some(tx) = &self.snapshot_tx {
            // We need to create a new receiver — the original one was moved into
            // the actor during construction. We create a new channel pair and
            // replace the sender.
    ...
    fn rebuild_cache(&mut self) -> Result<()> {
        ...
        let snapshot = graph_db.shared_graph().read();
    ...
    ///
    /// For `Unknown` type nodes (e.g., directories/files), the uniqueness
    /// constraint is relaxed to allow same-named nodes in different parts
    /// of the project tree (e.g., two `build_parsers` dirs). The caller
    /// (`StoreNode`) performs a more precise path-based check for these.
                ...
                if let Some(node_id) = snapshot.node_id_for_row(selene_db_graph::store::RowIndex::new(row)) {
                    if let Some(props) = snapshot.node_properties(node_id) {
                        ...
                        if type_match && name_match {
                            // For Unknown-type nodes (files/directories), skip the
                            // simple (type, name) check — the caller does a more
                            // precise path-based check.
    ...
    fn store_in_selene(&mut self, graph_node: &GraphNode) -> Result<NodeId> {
        ...
        let selene_id = graph_db.create_node(labels, properties)?;
        Ok(selene_id)
    }
    ...
    fn store_edge_in_selene(
        ...
        let edge_id = graph_db.create_edge(predicate, from_id, to_id, properties.to_vec())?;
        Ok(edge_id)
    }

    /// Schedule a debounced snapshot write.
    ///
    /// Sends a unit value through the `snapshot_tx` channel. The background
    /// task receives it, waits 2 seconds for any additional mutations to
    /// arrive (debounce), then writes a snapshot to disk. This ensures that
    /// rapid successive mutations only trigger one snapshot write.
    ...
    /// Spawn the background debounced snapshot task.
    ///
    /// This task listens on the `snapshot_rx` channel. Each time a unit value
    /// is received, it resets a 2-second debounce timer. When the timer fires
    /// without interruption, it writes a snapshot to disk.
                    ...
                    Some(_) => {
                        ...
                        match graph_db.write_snapshot(&data_dir, next_seq, true) {
                            Ok(outcome) => {
                                ...
                                if let Err(e) = graph_db.compact() {
                                    warn!("MemoryGraph snapshot task: compact failed: {}", e);
                                }
                            }
                            Err(e) => {
                                warn!("MemoryGraph snapshot task: write_snapshot failed: {}", e);
                            }
                        }
                    }
                    None => {
                        // Channel closed — task is being shut down
    ...
    async fn handle(&mut self, msg: Self::Message) {
        match msg {
            ...
            MemoryGraphMessage::Initialize { data_dir, reply_to } => {
                info!("MemoryGraph: initialize({})", data_dir.display());
            ...
            MemoryGraphMessage::InitializeEmbedder { model_path, embedder, reply_to } => {
                info!("MemoryGraph: initialize_embedder");
            ...
            MemoryGraphMessage::GetNode { id, reply_to } => {
                ...
                let graph_db = match self.graph_db.as_ref() {
                    Some(db) => db,
                    None => {
                        let _ = reply_to.send(Err(anyhow::anyhow!("GraphDb not initialized")));
                        return;
                    }
                };
            ...
            MemoryGraphMessage::StoreNode { node, reply_to } => {
                ...
                // Schedule a debounced snapshot to persist this mutation
                        ...
                        match snapshot.node_properties(selene_id) {
                            Some(props) => {
                                if let Some(existing) = Self::build_node_from_props(selene_id, &props) {
                                    ...
                                    if let Err(e) = graph_db.delete_node(selene_id) {
                    ...
                    Some(selene_id) => {
                        ...
                        for edge in &edges {
                            if let Some(edge_id) = self.uuid_to_edge.get(&edge.id).copied() {
                                let _ = graph_db.delete_edge(edge_id);
                                self.uuid_to_edge.remove(&edge.id);
                                self.edge_to_uuid.remove(&edge_id);
                            }
                        }

                        match graph_db.delete_node(selene_id) {
                            Ok(()) => {
                                self.uuid_to_node.remove(&id);
                                self.node_to_uuid.remove(&selene_id);
                                // Schedule a debounced snapshot to persist this mutation
            ...
            MemoryGraphMessage::DeleteRelationship { id, reply_to } => {
                ...
                let result = match edge_id {
                    Some(edge_id) => {
                        match graph_db.delete_edge(edge_id) {
                            Ok(()) => {
            ...
            MemoryGraphMessage::SearchContext { query, options, reply_to } => {
                ...
                let embedder = match self.embedder.as_ref() {
                    Some(e) => e,
                    None => {
                        let _ = reply_to.send(Err(anyhow::anyhow!("Embedder not initialized")));
                        return;
                    }
                };
                ...
                let hits = match graph_db.exact_vector_search(
                    LABEL_SPIRE_NODE,
                    "embedding",
            ...
            MemoryGraphMessage::AddMemory { text, metadata, reply_to } => {
                ...
                let _embedding = match embedder.embed(&text).await {
                    Ok(emb) => emb,
                    Err(e) => {
                        let _ = reply_to.send(Err(anyhow::anyhow!("Embedding failed: {}", e)));
                        return;
                ...
                let selene_id = match graph_db.create_node(labels, properties) {
                    Ok(id) => id,
                    Err(e) => {
                        let _ = reply_to.send(Err(e));
                        return;
            ...
            MemoryGraphMessage::SetConfig { key, value, reply_to } => {
                ...
                let result = (|| -> Result<()> {
                    ...
                    graph_db.create_node(vec![LABEL_CONFIG.to_string()], properties)?;
            ...
            MemoryGraphMessage::BootstrapMcpConfig { config_path, reply_to } => {
                info!("MemoryGraph: bootstrap_mcp_config({})", config_path.display());
                ...
                let result = (|| -> Result<()> {
                    ...
                    let config_file: McpConfigFile = serde_json::from_str(&content)
                        .map_err(|e| anyhow::anyhow!("Failed to parse MCP config file: {}", e))?;

                    // Check if MCP config nodes already exist (skip if bootstrapped)
                    let snapshot = graph_db.shared_graph().read();
                    let has_existing = if let Some(bitmap) = snapshot.nodes_with_label(&selene_db_core::db_string::DbString::try_from("SpireMcpConfig").unwrap()) {
                        bitmap.iter().next().is_some()
                    } else {
                        false
                    };
                    drop(snapshot);
                    ...
                    if has_existing {
                        info!("MemoryGraph: MCP config already bootstrapped, skipping");
                        return Ok(());
                    }

                    // Store each server as a SpireMcpConfig node
                    for server in &config_file.servers {
                        ...
                        let json_str = serde_json::to_string(&server_json)
                            .map_err(|e| anyhow::anyhow!("Failed to stringify server config: {}", e))?;

                        let properties = vec![
                            (PROP_NAME.to_string(), Value::String(selene_db_core::db_string::DbString::try_from(server.name.clone()).unwrap())),
                            (PROP_CONFIG_VALUE.to_string(), Value::String(selene_db_core::db_string::DbString::try_from(json_str).unwrap())),
                        ];
                        graph_db.create_node(vec!["SpireMcpConfig".to_string()], properties)?;
                    }

                    info!("MemoryGraph: bootstrapped {} MCP server configs", config_file.servers.len());
            ...
            MemoryGraphMessage::GetMcpConfig { reply_to } => {
                info!("MemoryGraph: get_mcp_config");
                ...
                let result = (|| -> Result<Vec<McpServerConfigEntry>> {
                    ...
                    if let Some(bitmap) = snapshot.nodes_with_label(&selene_db_core::db_string::DbString::try_from("SpireMcpConfig").unwrap()) {
                        for row in bitmap.iter() {
                            if let Some(node_id) = snapshot.node_id_for_row(selene_db_graph::store::RowIndex::new(row)) {
                                if let Some(props) = snapshot.node_properties(node_id) {
                                    if let Some(val_val) = props.get(&selene_db_core::db_string::DbString::try_from(PROP_CONFIG_VALUE).unwrap()) {
                                        if let Value::String(json_str) = val_val {
                                            if let Ok(server) = serde_json::from_str::<McpServerConfigEntry>(json_str.as_str()) {
                                                servers.push(server);
                                            }
                                        }
                                    }
            ...
            // ── ImportMcpConfigFile ─────────────────────
            MemoryGraphMessage::ImportMcpConfigFile { config_path, reply_to } => {
                info!("MemoryGraph: import_mcp_config_file({})", config_path.display());
                ...
                let result = (|| -> Result<()> {
                    ...
                    // Delete all existing SpireMcpConfig nodes
                    let snapshot = graph_db.shared_graph().read();
                    let existing_ids: Vec<NodeId> = if let Some(bitmap) = snapshot.nodes_with_label(&selene_db_core::db_string::DbString::try_from("SpireMcpConfig").unwrap()) {
                        bitmap.iter()
                            .filter_map(|row| snapshot.node_id_for_row(selene_db_graph::store::RowIndex::new(row)))
                            .collect()
                    } else {
                        Vec::new()
                    };
                    ...
                    for node_id in &existing_ids {
                        let _ = graph_db.delete_node(*node_id);
                    }

                    // Store each server as a new SpireMcpConfig node
                    for server in &config_file.servers {
                        let server_json = serde_json::to_value(server)
                            .map_err(|e| anyhow::anyhow!("Failed to serialize server config: {}", e))?;
                        let json_str = serde_json::to_string(&server_json)
                    ...
                    info!("MemoryGraph: imported {} MCP server configs from {}", config_file.servers.len(), config_path.display());
            ...
            // ── SaveMcpConfig ───────────────────────────
            MemoryGraphMessage::SaveMcpConfig { entry, reply_to } => {
                info!("MemoryGraph: save_mcp_config({})", entry.name);
                ...
                let result = (|| -> Result<()> {
                    ...
                    let server_json = serde_json::to_value(&entry)
                        .map_err(|e| anyhow::anyhow!("Failed to serialize server config: {}", e))?;
                    let json_str = serde_json::to_string(&server_json)
                        .map_err(|e| anyhow::anyhow!("Failed to stringify server config: {}", e))?;
                    ...
                    let snapshot = graph_db.shared_graph().read();
                    let existing_id = if let Some(bitmap) = snapshot.nodes_with_label(&selene_db_core::db_string::DbString::try_from("SpireMcpConfig").unwrap()) {
                        let mut found = None;
                        for row in bitmap.iter() {
                            if let Some(node_id) = snapshot.node_id_for_row(selene_db_graph::store::RowIndex::new(row)) {
                                if let Some(props) = snapshot.node_properties(node_id) {
                                    if let Some(name_val) = props.get(&selene_db_core::db_string::DbString::try_from(PROP_NAME).unwrap()) {
                                        if let Value::String(s) = name_val {
                                            if s.as_str() == entry.name {
                                                found = Some(node_id);
                                                break;
                                            }
                                        }
                    ...
                    // Delete existing node if present (replace)
                    ...
                    let properties = vec![
                        (PROP_NAME.to_string(), Value::String(selene_db_core::db_string::DbString::try_from(entry.name.clone()).unwrap())),
                        (PROP_CONFIG_VALUE.to_string(), Value::String(selene_db_core::db_string::DbString::try_from(json_str).unwrap())),
                    ];
                    graph_db.create_node(vec!["SpireMcpConfig".to_string()], properties)?;

                    info!("MemoryGraph: saved MCP server config '{}'", entry.name);
            ...
            // ── DeleteMcpConfig ─────────────────────────
            MemoryGraphMessage::DeleteMcpConfig { name, reply_to } => {
                info!("MemoryGraph: delete_mcp_config({})", name);
                ...
                let result = (|| -> Result<()> {
                    ...
                    let snapshot = graph_db.shared_graph().read();
                    let found_id = if let Some(bitmap) = snapshot.nodes_with_label(&selene_db_core::db_string::DbString::try_from("SpireMcpConfig").unwrap()) {
                        let mut found = None;
                        for row in bitmap.iter() {
                            if let Some(node_id) = snapshot.node_id_for_row(selene_db_graph::store::RowIndex::new(row)) {
                                if let Some(props) = snapshot.node_properties(node_id) {
                    ...
                    match found_id {
                        Some(node_id) => {
                            graph_db.delete_node(node_id)?;
                            info!("MemoryGraph: deleted MCP server config '{}'", name);
                            // Schedule a debounced snapshot to persist this mutation
                            self.schedule_snapshot();
                            Ok(())
                        }
                        None => {
                            Err(anyhow::anyhow!("MCP server config '{}' not found", name))
                        }
                    }
                })();
            ...
            MemoryGraphMessage::Sync { reply_to } => {
                ...
                // Write a snapshot to disk for cross-session persistence.
                // The snapshot captures the full graph state (nodes, edges, indexes)
                // and is used together with the WAL for crash recovery on restart.
                let result = (|| -> Result<()> {
                    let data_dir = self.data_dir.as_ref().ok_or_else(|| {
                        anyhow::anyhow!("data_dir not set")
                    })?;

                    // Determine the next snapshot sequence number
                    let next_seq = match GraphDb::latest_snapshot_sequence(data_dir)? {
                        Some(seq) => seq + 1,
                        None => 1,
                    };

                    // Write snapshot with fsync for durability
