...
impl McpClientManager {
    ...
    ///
    /// Priority:
    /// 1. `SPIRE_MCP_CONFIG` environment variable
    /// 2. `~/.spire/mcp-config.json`
    fn config_path() -> Result<PathBuf> {
        if let Ok(path) = std::env::var("SPIRE_MCP_CONFIG") {
        ...
        Ok(PathBuf::from(home).join(".spire").join("mcp-config.json"))
    }
    ...
    /// Load external server configurations from the MCP config file.
    ///
    /// Returns the path that was loaded, or `None` if the file doesn't exist.
    pub fn load_config(&mut self) -> Result<Option<PathBuf>> {
        ...
        let content = std::fs::read_to_string(&config_path)
            .context(format!("Failed to read MCP config from {}", config_path.display()))?;

        let config: serde_json::Value = serde_json::from_str(&content)
            .context("Failed to parse MCP config JSON")?;

        // Try the unified format first: { "mcpServers": { ... } }
        // Then fall back to { "external_servers": [...] }
        let servers: Vec<McpServerConfig> = if let Some(mcp_servers) = config.get("mcpServers").and_then(|v| v.as_object()) {
            // New format: { "mcpServers": { "server-name": { "command": "...", "args": [...], ... } } }
            mcp_servers.iter().map(|(name, server_config)| {
                let transport = if let Some(url) = server_config.get("url").and_then(|v| v.as_str()) {
                    ...
                    TransportConfig::Stdio {
                        command: server_config.get("command").and_then(|v| v.as_str()).unwrap_or("").to_string(),
                        ...
                        env: server_config.get("env")
                            .and_then(|v| v.as_object())
                            .map(|e| e.iter().map(|(k, v)| (k.clone(), v.as_str().unwrap_or("").to_string())).collect())
                            .unwrap_or_default(),
                    }
                };
                McpServerConfig {
                    name: name.clone(),
                    transport,
                    autostart: server_config.get("autostart").and_then(|v| v.as_bool()).unwrap_or(true),
                }
            }).collect()
        } else if let Some(external) = config.get("external_servers").and_then(|v| v.as_array()) {
            // Old format: { "external_servers": [ { "name": "...", "transport": { "type": "stdio", ... } } ] }
            external.iter().filter_map(|v| {
                let name = v.get("name")?.as_str()?.to_string();
                let transport_val = v.get("transport")?;
                let transport_type = transport_val.get("type")?.as_str()?;
                let transport = match transport_type {
                    "stdio" => TransportConfig::Stdio {
                        command: transport_val.get("command")?.as_str()?.to_string(),
                        args: transport_val.get("args")
                            .and_then(|a| a.as_array())
                            .map(|a| a.iter().filter_map(|v| v.as_str().map(String::from)).collect())
                            .unwrap_or_default(),
                        env: transport_val.get("env")
                            .and_then(|e| e.as_object())
                            .map(|e| e.iter().map(|(k, v)| (k.clone(), v.as_str().unwrap_or("").to_string())).collect())
                            .unwrap_or_default(),
                    },
                    "http" => TransportConfig::Http {
                        url: transport_val.get("url")?.as_str()?.to_string(),
                        headers: transport_val.get("headers")
                ...
                Some(McpServerConfig {
                    name,
                    transport,
                    autostart: v.get("autostart").and_then(|a| a.as_bool()).unwrap_or(true),
                })
            }).collect()
        } else {
            vec![]
